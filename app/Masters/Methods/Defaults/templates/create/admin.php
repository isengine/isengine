...
Objects::each($message['data'], function ($item, $key) use (&$content) {
    $content .= '<tr>
    <td>' . $key . ':</td>
    <td>' . (
        !empty($item)
...
<div style="font-size: 10px; color: #777777;">
	Новый заказ на сайте ' . System::server('host') . '
</div>
...
<div style="font-size: 21px;">
    Опубликован новый заказ на ' . System::server('host') . '
</div>


<div style="line-height: 1.5em;">
    <p>Номер заказа: <strong>' . $message['name'] . '</strong></p>
    <p>Автор заказа: <strong>' . Strings::join($message['owner'], ', ') . '</strong></p>
